...
def init_socket_events(socketio):
    ...
    Args:
        socketio: Flask-SocketIO 實例
    """
    @socketio.on('connect')
    def handle_connect():
        """處理客戶端連接事件"""
        from flask import session
        from services.game_service import rooms
    ...
    def handle_player_ready():
        """處理玩家準備狀態"""
        from flask import session
        from services.game_service import rooms, start_game
    ...
    def handle_answer(data):
        """處理玩家提交答案"""
        from flask import session, request
        from services.game_service import submit_answer
        ...
        # 提交答案並獲取結果
        result = submit_answer(username, room_id, answer)
...
def update_room_status(room_id, room_data):
    """更新並廣播房間狀態
    ...
    emit('room_status', {
        ...
        'ready': room_data['ready'],
        'game_started': room_data['game_started'],
        'ratings': ratings,
        'game_mode': room_data['game_mode'],
        'is_ranked': room_data.get('is_ranked', False),
        'auto_start': room_data.get('auto_start', False)
    }, room=room_id)

def notify_user_left(username, room_id):
    """通知房間其他人某玩家離開
...
def handle_question_timeout(socketio, room_id, question_number):
    """處理問題時間到期
    
    Args:
        socketio: Flask-SocketIO 實例
        room_id: 房間ID
        question_number: 問題編號，用於確認是否仍在同一問題
    """
    from services.game_service import rooms, next_question
    import time
    
    # 等待指定的遊戲時間
    time.sleep(int(rooms[room_id]['game_time']))
